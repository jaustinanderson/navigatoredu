...
author edits pack ─► validate_pack (contract check, no DB)
                        ─► seed.py (idempotent upsert into SQLite)
...
3. **Validate** — `python -m backend.app.validate_pack <file>` (details
   below).
4. **Seed** — `SEED_PATH=<file> python -m backend.app.seed` creates tables if
   missing and upserts every record via `session.merge()`, so re-running
   after an edit converges the database. The pack's governance metadata is
   upserted into a single-row `PackMetadata` table (fixed `id=1`).
5. **Serve** — the app reports the loaded pack at
   `GET /api/v1/pack-metadata` and in the UI banner.

Packs share an ID scheme, so switch domains on a fresh database — the
upserting seed would otherwise merge two domains.
...
**Frontend.** A single `frontend/index.html` (~300 lines): Tailwind via CDN,
